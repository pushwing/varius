...
- **시·도 GeoJSON 을 먼저 판별**한다 — 매치되면 즉시 `KR` + `region_code`. 시·도 경계가
  세계 110m 경계보다 해상도가 높아, 한국 해안 사진이 거친 국가 경계 때문에 바다로
  새는 오판을 원천 차단한다. 시·도 미매치 시에만 세계 경계로 국가 판별.
...
- 110m 해상도는 해안선이 거칠어 해안 사진이 바다로 판정될 수 있다 — 한국은 시·도
  우선 판별로 해결되고, 해외 해안의 드문 오판(null)은 통계 제외로 수용한다.
  실사용에서 문제가 되면 그때 판별용 50m 파일 추가를 검토한다(YAGNI).
