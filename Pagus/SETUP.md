...
- Groq API 키(관리자 화면의 AI 카테고리 추천을 사용할 때)
...
관리자 맛집 등록·수정 화면에서 상호에 맞는 카테고리를 추천하려면 [Groq Console](https://console.groq.com/)에서 API 키를 발급받는다. Groq API 키는 서버에서만 사용하며 브라우저에 노출하지 않는다.

Pagus 디렉터리의 `.env`에 다음 값을 입력한다.

```dotenv
# Groq Console에서 발급한 서버용 API 키
groq.apiKey = 발급받은_Groq_API_키

# 선택 사항: 기본값은 llama-3.1-8b-instant
# groq.model = llama-3.1-8b-instant
```

- 운영자 맛집 등록·수정 화면에서 상호를 입력하고 `AI 카테고리 추천`을 누르거나 상호 입력란을 벗어나면 추천을 요청한다.
- 추천 대상은 현재 활성화된 카테고리뿐이며, 추천 결과는 운영자가 저장 전에 직접 확인·수정해야 한다.
- API 키가 없거나 호출이 실패하거나 유효하지 않은 카테고리 ID가 반환되면 카테고리를 자동 저장하지 않고 직접 선택할 수 있게 한다.
- Groq API 키와 응답 내용은 로그·화면·커밋에 남기지 않는다.
