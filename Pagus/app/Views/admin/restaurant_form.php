...
(() => {
    ...
    const recommendCategory = async () => { const query = name.value.trim(); if (query.length < 1 || query.length > 150) { categoryRecommendMessage.textContent = '상호를 1자 이상 150자 이하로 입력하세요.'; return; } categoryRecommendMessage.textContent = 'AI 카테고리 추천 중...'; try { const response = await fetch('/admin/restaurants/recommend-category?name=' + encodeURIComponent(query), {headers: {'Accept': 'application/json'}}); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || 'AI 카테고리 추천을 사용할 수 없습니다.'); const ids = new Set((payload.category_ids || []).map((id) => String(id))); document.querySelectorAll('input[name="category_ids[]"]').forEach((input) => { if (ids.has(input.value)) input.checked = true; }); categoryRecommendMessage.textContent = ids.size > 0 ? '추천 카테고리를 선택했습니다. 저장 전 직접 확인하세요.' : '적합한 카테고리를 찾지 못했습니다. 직접 선택하세요.'; } catch (error) { categoryRecommendMessage.textContent = error.message || 'AI 카테고리 추천을 사용할 수 없습니다. 카테고리를 직접 선택하세요.'; } };
