...
- **외부용 앱**: 바닐라 JS + Vite + `livekit-client`(`web/` 참고). 프레임워크 없이 최소 구성 — 가족용 인터콤 규모에 React 등은 과함.
...
- CI/CD가 없으므로 머지 전 아래를 **로컬에서** 직접 실행해 확인한다.
  ...
  - 외부용 앱(`web/`): `npm run lint` + `npm run format:check` + `npm run test`(Vitest), 한 번에 `npm run ci`(`web/README.md` 참고)
