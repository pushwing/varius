...
- 테이블: `photo_locations`(좌표·시간·`google_media_item_id`), `oauth_tokens`(refresh/access 토큰 **암호화** 저장). 상세 스키마는 명세 6절 참고.
- **원본 이미지 파일은 저장하지 않는다.** EXIF 추출 후 임시 파일을 즉시 폐기하고 좌표·시간·`media_item_id`만 DB에 저장한다.
- 사진 자체를 다시 표시해야 하면 그 시점에 API로 재조회해 새 `baseUrl`을 발급받는다(캐싱 불가).
